...
* 또한, 해당 알고리즘을 편리하게 사용할 수 있도록 PyQt를 사용하여 PC Application을 개발함
...
<table align = 'center'>
 ...
 <tr>
  ...
  <td>화면 중앙의 적색 사각형 안에 손이 처음으로 감지될 때부터 사각형 밖으로 손이 나갈 때까지의 전체 프레임 수를 확인한다</td>
 </tr>
 <tr>
  <!--<td>행동 판단 조건 </td>-->
  <td>전체 프레임 중 LSTM 모델이 각 단계에서 필요로 하는 동작을 탐지한 프레임의 수가 사전에 정한 비율 이상일 시 행동을 수행하였다고 판단한다</td>
 </tr>
 <tr>
  <!--<td>행동 판단 조건 </td>-->
 ...
 <tr>
  ...
  <td>정면(내려놓기) → 옆면 및 뒷면(회전하기) → 윗면 → 아랫면 순으로 진행하며, 물건의 상태나 손동작이 각 단계에서 요구하는 조건을 충족하지 못할 경우 재수행을 요구한다</td>
 </tr>
 <tr>
  <!--<td>절차 인식 조건 </td>-->
